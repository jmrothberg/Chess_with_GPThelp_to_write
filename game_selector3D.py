...
font_big = pygame.font.Font("ARIALUNI.TTF", 128)
...
def display_saved_games():
    ...
    for i, file in enumerate(files):
        ...
        y = (i // icons_per_row) * (icon_size+4) + 4 + SCREEN_HEIGHT - 200
...
def draw_pieces_not_on_board(screen, board, height=SCREEN_HEIGHT):
    ...
    pygame.draw.rect(screen, LIGHT_GRAY, (0, height - 200, 24,200))
    pygame.draw.rect(screen, LIGHT_GRAY, (SCREEN_WIDTH - 24, height - 200, 24,200))
    # Draw the pieces not on the board
    for color, pieces in full_set.items():
        if len(pieces) > 8:
            size_div = 2
            font = pygame.font.Font("ARIALUNI.TTF", int(20/size_div))
        for i, piece in enumerate(pieces):
            if color == "B":
                x = 0 + 4
                y = height -196 + i * 22/size_div
            else:
                x = SCREEN_WIDTH - 24/size_div
                y = height -196 +  i * 22/size_div
            if piece[0] == "W":
...
def loop_to_select_new_game(screen, board, height=SCREEN_HEIGHT):
    #reset pygame display
    pygame.display.set_mode((SCREEN_WIDTH, SCREEN_HEIGHT))
    #want screen rectangle to be white and all but 200 of bottom to be gray
    rect_height = 200
    pygame.draw.rect(screen, WHITE, (0, height - rect_height, SCREEN_WIDTH, rect_height))
    pygame.display.set_caption("Use Mouse to Select, Hit return to exit")
    position = [0, 0]
    selected_info = None
    selected_game = None
    choosing_game = True
    pygame.draw.rect(screen, LIGHT_GRAY, (0, height - rect_height, SCREEN_WIDTH, rect_height))
    ...
    while choosing_game:
        ...
        if new_position is not None and new_position != "exit":
            ...
            if number < len_boards:
                ...
                draw_pieces_not_on_board(screen, selected_game, height=height)
                #display_saved_games()
                pygame.display.flip()
