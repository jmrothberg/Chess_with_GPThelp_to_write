...
def _train_chess_model_core(text, checkpoint_data=None, token_mode='4token'):
    ...
    if torch.cuda.is_available():
        ...
        if gpu_indices and len(gpu_indices) > 0:
            ...
            # Set CHESS_NO_COMPILE=1 to disable if driver/VNC crashes occur
            if os.environ.get('CHESS_NO_COMPILE'):
                print("ℹ️  torch.compile() disabled via CHESS_NO_COMPILE env var")
            elif device.type in ['mps', 'cuda'] and hasattr(torch, 'compile'):
                try:
                    ...
                    if max_requested_gpu >= available_gpus:
                        ...
                        for i, gpu_idx in enumerate(gpu_indices):
                            ...
                            if checkpoint_data:
                                ...
                                # Abort immediately if checkpoint has NaN weights
                                if i == 0:
                                    for pname, pval in cleaned_state.items():
                        ...
                        print(f"✅ Custom multi-GPU setup complete: {len(models)} models on GPUs {gpu_indices}")
                        ...
                        try:
                            ...
                            if min_headroom > 0.20:
                                print(f"Memory headroom {min_headroom:.0%} > 20% — disabling gradient checkpointing for speed")
                                for m in models:
                                    m.set_gradient_checkpointing(False)
                            else:
                                print(f"Memory headroom {min_headroom:.0%} <= 20% — keeping gradient checkpointing enabled")
                        except Exception as e:
                            print(f"Could not check memory headroom: {e}, keeping gradient checkpointing enabled")
            ...
            if checkpoint_data and use_custom_parallel:
                ...
                # Only load optimizer state for GPU 0 — it's the only optimizer that steps.
                # GPUs 1-3 only contribute gradients; their optimizers are never used.
                opt_state_0 = opt_states[0] if opt_states else None
                if opt_state_0 is not None:
                    try:
                        optimizers[0].load_state_dict(opt_state_0)
                        print(f"  GPU 0: optimizer state loaded (master optimizer)")
                        if scheduler_choice == 'cosine':
                            optimizers[0].param_groups[0]['lr'] = learning_rate
                            print(f"  GPU 0: reset LR to {learning_rate} for cosine scheduler")
                    except ValueError as e:
                        if "parameter group" in str(e):
                            print(f"  ⚠️  GPU 0: Optimizer state incompatible, using fresh state")
                        else:
                            raise e
                print(f"  GPUs 1-{len(optimizers)-1}: using fresh optimizers (only GPU 0 steps)")
            ...
            if not use_custom_parallel:
                # Single GPU mode
                print(f"✅ SINGLE GPU MODE: Using GPU {gpu_indices[0]}")
                use_custom_parallel = False
                ...
                try:
                    ...
                    if headroom > 0.20:
                        ...
                        print(f"Memory headroom {headroom:.0%} <= 20% — keeping gradient checkpointing enabled")
                except Exception as e:
                    print(f"Could not check memory headroom: {e}, keeping gradient checkpointing enabled")
    ...
    if use_custom_parallel:
        ...
        while epoch < num_epochs:
            ...
            for batch_idx, (x, y, y_roles) in enumerate(data_loader):
                ...
                gpu_losses_tensors = [None] * num_gpus

                # Forward+backward on all GPUs using BF16 autocast (no GradScaler needed).
                # BF16 has the same exponent range as FP32, so no loss scaling required.
                # GradScaler is fundamentally single-device — using it across GPUs causes
                # scale factor mismatches and NaN divergence.
                for local_idx, (model_gpu, opt_gpu, x_gpu, y_gpu, yr_gpu) in enumerate(zip(models_single, optimizers_single, x_splits, y_splits, yr_splits)):
                    ...
                    gpu_losses_tensors[local_idx] = loss.detach()

                # Synchronize all devices before cross-GPU gradient access
                for i in range(num_gpus):
                    torch.cuda.synchronize(gpu_indices_single[i])

                # Collect losses AFTER sync
                gpu_losses = [l.item() for l in gpu_losses_tensors]
                total_loss = sum(gpu_losses)

                # Average gradients onto GPU 0 (only GPU 0's optimizer steps)
                if num_gpus > 1:
                    with torch.no_grad():
                        for param_tuple in zip(*[m.parameters() for m in models_single]):
                            p0 = param_tuple[0]
                            if p0.grad is None:
                                continue
                            for p in param_tuple[1:]:
                                if p.grad is not None:
                                    # Route through CPU — P2P GPU transfers are broken on this system
                                    p0.grad.data.add_(p.grad.data.cpu().to(p0.grad.device))
                            p0.grad.data.div_(num_gpus)

                # Clip gradients on GPU 0 (the only model whose optimizer steps)
                torch.nn.utils.clip_grad_norm_(models_single[0].parameters(), clip_threshold)

                # Debug summary every 1000 batches
                if batch_idx % 1000 == 0 and batch_idx > 0:
                    gpu_grad_counts = [sum(1 for p in m.parameters() if p.grad is not None) for m in models_single]
                ...
                # Step only GPU 0's optimizer (master). GPUs 1-3 only contribute gradients.
                # This avoids all cross-device optimizer state issues.
                optimizers_single[0].step()

                # Copy GPU 0's updated weights to all other GPUs
                if num_gpus > 1:
                    with torch.no_grad():
                        for param_tuple in zip(*[m.parameters() for m in models_single]):
                            src = param_tuple[0].data
                            for p in param_tuple[1:]:
                                # Route through CPU — P2P GPU transfers are broken on this system
                                p.data.copy_(src.cpu().to(p.device))
                ...
                if (batch_idx + 1) % inference_frequency == 0:
                    ...
                    gpu_loss_str = ' | '.join(f'GPU{i}:{gpu_losses[i]:.3f}' for i in range(num_gpus))
                    print(f"Epoch [{epoch+1}/{num_epochs}], Batch [{batch_idx+1}/{len(data_loader)}], Loss: {avg_running_loss:.4f} [{gpu_loss_str}]")
